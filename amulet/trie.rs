...
use std::oldmap::HashMap; // XXX std::oldmap is deprecated

pub struct Trie<K, V> {
    value: option::Option<V>,
    children: @HashMap<K, @mut Trie<K, V>>,
}
...
pub fn Trie<T: Eq + IterBytes + Hash + Copy + Const, U: Copy>() -> @mut Trie<T, U> {
    return @mut Trie{ value: option::None::<U>, children: @HashMap() };
}
    ...
    pub fn insert(@mut self, keys: &[T], value: U) {
        ...
        for uint::range(0, keys.len()) |k| {
            if node.children.contains_key(&keys[k]) {
                ...
                let mut new_node = @mut Trie{ value: option::None::<U>, children: @HashMap() };
